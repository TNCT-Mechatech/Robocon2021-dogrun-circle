...
double yaw_move(std::vector<DetectionData> _datas, int _array_num, double _size, double _target_size){
...
int off_screen(double _yaw, double _size, double _target_size){
    int flg;
    if( _yaw > 0 && _size < _target_size) {
        flg = 1;
    } else if( _yaw < 0 && _size < _target_size){
        flg = 2;
    } else if( _yaw>0&& _size > _target_size){
        flg = 3;
    } else if( _yaw < 0 && _size > _target_size){
    ...
    if(flg == 0){
        if(!ball_datas.empty()){
            ...
            if(array_co != 0){
                ...
                yaw = yaw_move(ball_datas, array_num, size, 50000);
                ofsc_flg = off_screen(yaw, size, 50000);
                if (0.3 > yaw && yaw > -0.3){
